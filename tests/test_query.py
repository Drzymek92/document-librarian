...
def test_query_returns_ranked_hits(tmp_path, sample_dir, monkeypatch):
    ...
    results, mode = query.search(con, "fox")
    assert mode == "bm25"
    assert results
...
def test_query_type_filter(tmp_path, sample_dir, monkeypatch):
    ...
    results, _ = query.search(con, "fox", types=["docx"], limit=20)
    assert results
...
def test_query_source_filter(tmp_path, sample_dir, monkeypatch):
    con = _ingested(tmp_path, sample_dir, monkeypatch)
    assert query.search(con, "fox", source="gdrive", limit=20)[0] == []
    assert query.search(con, "fox", source="local", limit=20)[0]
...
def test_query_tag_filter(tmp_path, sample_dir, monkeypatch):
    ...
    hit = query.search(con, "fox", limit=20)[0][0]
    db.set_tags(con, hit["doc_id"], ["special-marker"])
    results, _ = query.search(con, "fox", tags=["special-marker"], limit=20)
    assert results
